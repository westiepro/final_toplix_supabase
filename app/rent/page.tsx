...
export default function RentPage() {
  ...
  const applyFilters = useCallback(() => {
    ...
    if (filters.features && filters.features.length > 0) {
      // For now, we'll just pass through since properties don't have features yet
      // This can be implemented when features are added to the Property type
    }
  ...
  const loadProperties = async () => {
    try {
      setLoading(true)
      const data = await fetchPropertiesByType('rent')
      setProperties(data)
            ...
            <div className="mb-4 flex items-center justify-between gap-4">
              <h1 className="text-2xl font-bold flex items-center gap-2">
                {filters.city 
                  ? (
                    <>
                      Properties for rent in {filters.city.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}
                      {filters.country && (
